...
const pageTitles = {
  ...
  qc: "ตรวจสอบคุณภาพน้ำ (QC)",
  suppliers: "ซัพพลายเออร์",
  "purchase-orders": "ใบสั่งซื้อวัตถุดิบ",
  expenses: "ค่าใช้จ่าย",
  maintenance: "เครื่องจักร & ซ่อมบำรุง",
  deposits: "ถังมัดจำ",
...
function navigate(page) {
  ...
  const loaders = {
    ...
    "purchase-orders": { load: loadPurchaseOrders },
    expenses: { load: loadExpenses },
    maintenance: { load: loadMaintenance },
...
function loadSettingsPage() {
  ...
  document.getElementById("sys-login-time").textContent = new Date(
    localStorage.getItem("tws_login_time") || Date.now(),
...
window.addPOItem = () => {
  ...
  div.innerHTML = `
    ...
    <div class="col-1"><button class="btn btn-outline-danger btn-sm" onclick="this.closest('.po-item').remove();calcPOTotal()"><i class="bi bi-x"></i></button></div>`;
